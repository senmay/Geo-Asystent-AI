...
def test_process_query_gis_intent(mock_get_layer, mock_routing_chain):
    """
    Tests if the function correctly calls the GIS tool when intent is 'get_gis_data'.
    """
    # Arrange: Configure our mocks
    mock_routing_chain.invoke.return_value = {"intent": "get_gis_data", "layer_name": "parcels"}
...
def test_process_query_chat_intent(mock_chat_chain, mock_routing_chain):
    """
    Tests if the function correctly calls the chat chain when intent is 'chat'.
    """
    # Arrange
    mock_routing_chain.invoke.return_value = {"intent": "chat"}
...
@patch('services.agent_service.routing_chain')
def test_process_query_gis_intent_no_layer(mock_routing_chain):
    """
    Tests the response when the GIS intent is detected but no layer name is provided.
    """
    # Arrange
    mock_routing_chain.invoke.return_value = {"intent": "get_gis_data", "layer_name": None}
    ...
    # Assert
    assert result["type"] == "text"
    assert "nie sprecyzowałeś które" in result["data"]
